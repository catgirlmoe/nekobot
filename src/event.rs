...
impl EventHandler for Handler {
  ...
  async fn guild_member_addition(&self, ctx: Context, _guild_id: GuildId, member: Member) {
    ChannelId(GENERAL).send_message(&ctx.http, |m| {
      m.embed( |e| {
        let u = member.user;
        e.author( |a| {
  ...
  async fn guild_member_removal(&self, ctx: Context, _guild_id: GuildId, u: User, _member: Option<Member>) {
    ChannelId(GENERAL).send_message(&ctx.http, |m| {
      m.embed( |e| {
        e.author( |a| {
          a.icon_url(u.avatar_url().expect("Error retrieving avatar"));
  ...
  async fn reaction_add(&self, ctx: Context, r: Reaction) {
    if r.user_id.expect("Failed to get user id") == BOTID {return;}
    match r.message_id.as_u64() {
      COLOR_MSG => {
        give_role(COLOR_ROLES, COLOR_REACTS, ctx, r).await
      },
      OPT_MSG => {
        give_role(OPT_ROLES, OPT_REACTS, ctx, r).await
      },
      ACCESS_MSG => {
        give_role(ACCESS_ROLES, ACCESS_REACTS, ctx, r).await
      },
      CC_MSG => {
        give_role(CC_ROLES, CC_REACTS, ctx, r).await
      },
      _ => {}
    }
  }
  ...
  async fn reaction_remove(&self, ctx: Context, r: Reaction) {
    ...
    match r.message_id.as_u64() {
      COLOR_MSG => {
        take_role(COLOR_ROLES, COLOR_REACTS, ctx, r).await
      },
      OPT_MSG => {
        take_role(OPT_ROLES, OPT_REACTS, ctx, r).await
      },
      ACCESS_MSG => {
        take_role(ACCESS_ROLES, ACCESS_REACTS, ctx, r).await
      },
      CC_MSG => {
        take_role(CC_ROLES, CC_REACTS, ctx, r).await
      },
      _ => {}
    }
  }
